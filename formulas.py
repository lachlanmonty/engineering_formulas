...
def bingham_darcy_friction_factor(d_m, v, rho, t_0, mu_b):
    """Calculates the Darcy friction factor for a Bingham plastic.
    ...
    for i in range(10):
        f_L = (
            16
            / N_re
            * (1 + 1 / 6 * (N_he / N_re) - ((N_he ** 4) / (3 * f_L ** 3 * N_re ** 7)))
        )
...
def oroskar_settling_velocity(D, d, S, C_c, p_f, u_f, X=1, g=9.81):
    ...
    """Calculates the settling or critical velocity for a slurry with a 
...
def api_standard_520_part1_sizing(d_m, v, rho, t_0, mu_b):
    """Calculates required size for rupture disc with liquid medium. [1]
...
def cema_area(BW, beta, phi):
    ...
    A_s = (
        2 * BW ** 2 * (rsch ** 2 * ((phi / 2) - ((math.sin(phi) * math.cos(phi)) / 2)))
